...
x1 = [x1_min, x1_min, x1_min, x1_min, x1_max, x1_max, x1_max, x1_max, -1.73 * dx1 + x01, 1.73 * dx1 + x01, x01, x01,
      x01, x01, x01]
x2 = [x2_min, x2_min, x2_max, x2_max, x2_min, x2_min, x2_max, x2_max, x02, x02, -1.73 * dx2 + x02, 1.73 * dx2 + x02,
      x02, x02, x02]
x3 = [x3_min, x3_max, x3_min, x3_max, x3_min, x3_max, x3_min, x3_max, x03, x03, x03, x03, -1.73 * dx3 + x03,
      1.73 * dx3 + x03, x03]
...
def func(X1, X2, X3):
    y = 7.9 + 2.1 * X1 + 5.3 * X2 + 3.0 * X3 + 8.1 * X1 * X1 + 1.0 * X2 * X2 + 8.4 * X3 * X3 + 7.2 * X1 * X2 + 0.8 * \
        X1 * X3 + 2.3 * X2 * X3 + 6.4 * X1 * X2 * X3 + randrange(0, 10) - 5
    return y
...
def finds_value(num):
    b = 0
    for j in range(15):
        b += aver_y[j] * tmp_list_a[j][num - 1] / 15
    return b
...
def a(fs, s):
    c = 0
    for j in range(15):
        c += tmp_list_a[j][fs - 1] * tmp_list_a[j][s - 1] / 15
    return c
...
determinant2 = [my, finds_value(1), finds_value(2), finds_value(3), finds_value(4), finds_value(5), finds_value(6),
                finds_value(7),
...
for k in range(15):
    y_i[k] = beta[0] + beta[1] * tmp_list_a[k][0] + beta[2] * tmp_list_a[k][1] + beta[3] * tmp_list_a[k][2] + \
             ...
             tmp_list_a[k][6] + beta[8] * tmp_list_a[k][7] + beta[9] * tmp_list_a[k][8] + beta[10] * tmp_list_a[k][
                 9]
