...
x2 = [x2_min, x2_min, x2_max, x2_max, x2_min, x2_min, x2_max, x2_max, x02, x02, -1.73 * delta_x2 + x02,
      1.73 * delta_x2 + x02, x02, x02, x02]
x3 = [x3_min, x3_max, x3_min, x3_max, x3_min, x3_max, x3_min, x3_max, x03, x03, x03, x03, -1.73 * delta_x3 + x03,
      1.73 * delta_x3 + x03, x03]
x1x2 = [0] * 15
x1x3 = [0] * 15
x2x3 = [0] * 15
x1x2x3 = [0] * 15
...
def function(X1, X2, X3):
    y = 7.9 + 2.1*X1 + 5.3*X2 + 3.0*X3 + 8.1*X1*X1 + 1.0*X2*X2 + 8.4*X3*X3 + 7.2*X1*X2 + 0.8*X1*X3 + 2.3*X2*X3 + \
...
print("----------------Перевірка значущості "
      "коефіцієнтів за критерієм Стьюдента----------------")
